...
impl MqttClient {
    ...
    pub async fn run_event_loop(mut self, status_tx: mpsc::Sender<MqttStatus>) {
        ...
        while let Some(gps_data) = gps_rx.recv().await {
            ...
            if let Some(ref mut detector) = lap_detector {
                if let (Some(lat), Some(lon)) =
                    (gps_data.navigation.latitude, gps_data.navigation.longitude)
                {
                    if let Some(lap_data) = detector.update(lat, lon) {
                        if let Err(e) = publish_lap_data(
                            &publish_client,
                            &base_topic,
                            &lap_data,
                            &last_published,
        ...
        "gpx" => {
            ...
            if let Some(ref gpx_path) = config.track_gpx_file {
                match std::fs::read_to_string(gpx_path) {
                    Ok(gpx_content) => match parse_gpx_file(&gpx_content) {
                        Ok(points) => {
